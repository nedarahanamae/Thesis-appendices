...
Cluster_by_descrip<-IdClusters(DistMatr_by_descrip, method="UPGMA", cutoff=0.00512, showPlot=TRUE, myXStringSet=dna, verbose=TRUE) #will be displayed with taxon name for checking, but not later used as element
DistMatr_by_row<-DistanceMatrix(MSAAlignment_by_row_String, correction="none", verbose=FALSE)
Cluster_by_row <-IdClusters(DistMatr_by_row, method="UPGMA", cutoff=0.00512, myXStringSet=dna, verbose=TRUE) #will be displayed with row name for checking and later used as element

write.xlsx(DistMatr_by_descrip, col.names=TRUE, "E:/0_Master_Thesis/R_test/Final_R/28S_8Provinces/2bp/DistMatr_by_descrip.xlsx")
...
print("continue with  05_IPD_4_Analyses_28S_8Provinces_2bp")
#### continue with  05_IPD_4_Analyses_28S_8Provinces_2bp
